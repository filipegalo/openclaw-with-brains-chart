...
| tools.image.repository | string | `"alpine"` | Alpine image for init-tools container |
| tools.image.tag | string | `"3.21"` | Alpine image tag |
| kubectlIntegration.enabled | bool | `true` | Enable kubectl download and ClusterRole/Binding |
| githubIntegration.enabled | bool | `true` | Enable gh CLI download |
| githubIntegration.tokenSecret | string | `"openclaw-github-token"` | Secret name containing `GITHUB_TOKEN` |
| gateway.bind | string | `"lan"` | Network interface to bind (`lan` or `0.0.0.0`) |
| gateway.port | int | `18789` | Gateway listen port |
| chromium.enabled | bool | `false` | Enable headless Chromium sidecar |
| chromium.debugPort | int | `9222` | Chromium remote debugging port |
| replicaCount | int | `1` | Replicas (must be 1 — see edge cases) |
| strategy | string | `"RollingUpdate"` | Deployment strategy |
| workspace.enabled | bool | `true` | Enable git-backed workspace sync |
| workspace.repo | string | `""` | SSH URL of the brain repository (**required** when enabled) |
| workspace.branch | string | `"main"` | Branch to clone and push to |
| workspace.path | string | `"workspaces"` | Repo path containing workspace subdirectories |
| workspace.configPath | string | `"openclaw.json"` | Repo path to openclaw.json |
| workspace.syncInterval | int | `60` | Sync frequency in seconds (minimum 10) |
| workspace.gitUserName | string | `"openclaw"` | Git committer name |
| workspace.gitUserEmail | string | `"openclaw@example.com"` | Git committer email |
| workspace.sshKeySecret | string | `"openclaw-ssh-key"` | Secret name containing `id_ed25519` |
| telegramAllowFrom | list | `[]` | Telegram sender IDs allowed to interact (strings, not ints) |
| envFrom | list | see values.yaml | Secret/ConfigMap refs for environment variables |
| env | list | `[]` | Extra environment variables for the main container |
| networkPolicy.enabled | bool | `false` | Enable NetworkPolicy |
| ingress.enabled | bool | `false` | Enable Ingress |
| service.type | string | `"ClusterIP"` | Service type |
| service.port | int | `18789` | Service port |
